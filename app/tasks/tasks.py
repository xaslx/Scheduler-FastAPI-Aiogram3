...
send_notification_for_all_users, register_confirmation_template, 
 ...
 success_update_password, add_new_client, cancel_booking, get_help, confirm_booking
 )
...
@celery.task
def cancel_client(message: str, email: EmailStr, date: str, time: str, description: str):
    msg_content = cancel_booking(message=message, email_to=email, date=date, time=time, description=description)
...
@celery.task
def confirm_booking_for_client(email_to: EmailStr, tg: str, em: EmailStr, time: str, date: str):
