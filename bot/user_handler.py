...
async def create_new_booking(message: Message, state: FSMContext):
    ...
    await state.update_data(
        tg_username=(
            message.from_user.username if message.from_user.username else "Скрыт"
        )
    )
...
async def set_date(message: Message, state: FSMContext):
    ...
    try:
        ...
        user: UserOut = await BotService.find_user(
            personal_link=result["personal_link"]
        )
        await state.update_data(
            user_email=user.email,
            user_tg=user.telegram_link if user.telegram_link else "Не указан",
        )
        booking: BookingOut = await BotService.get_booking(
            user_id=user.id, date=formated_date
        )
        ...
        if not booking:
            ...
            booking_id: int = await BotService.add_booking(
                date_for_booking=result["date"], user_id=user.id, times=intervals
            )
            booking: BookingOut = await BotService.get_booking(
                user_id=user.id, date=result["date"]
            )
        ...
        if len(available_times) > 0:
            inline_kb = create_inline_button_times(
            ...
            return await message.answer(
                "На выбранную дату не осталось свободного времени"
            )
...
async def set_time(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(
        booking_id=booking_id, user_id=user_id, time=f"{hour}:{minute}"
    )
    await callback.message.edit_text(
        f"Вы выбрали время: <b>{hour}:{minute}.</b> (МСК)\nТеперь введите ваше имя.\nИли /cancel - если хотите отменить"
    )
    await state.set_state(NewBooking.name)


@user_router.message(
    StateFilter(NewBooking.name), (F.text & F.text.regexp(r"^.{2,20}$"))
)
...
@user_router.message(
    StateFilter(NewBooking.phone_number), (F.text & F.text.regexp(r"^.{10,11}$"))
)
async def set_phone_number(message: Message, state: FSMContext):
    ...
    scheduler.add_job(
        ...
        trigger="date",
        args=[
            message.from_user.id if message.from_user.id else "Не указан",
            res["time"],
        ],
        run_date=reminder_time,
...
async def cancel_booking(callback: CallbackQuery):
    ...
    if not user:
        await callback.message.answer(
            "Вы не подключили свой телеграм к профилю на сайте"
        )
    else:
...
async def confirm_cancel_booking(callback: CallbackQuery):
    ...
    formatted_time: str = str(
        time(hour=int(hour), minute=int(minute)).strftime("%H:%M")
    )
    user: UserOut = await BotService.find_user(telegram_id=callback.from_user.id)
    await callback.answer()
    if not user:
        ...
        if not booking:
            ...
            for client in booking.selected_times:
                if client[0] == formatted_time:
                    ...
                    break
        try:
            await BotService.cancel_booking(
                user_id=user.id, booking_id=booking.id, time=formatted_time
            )
        except:
            ...
            if cancel_booking.tg_id:
                await bot.send_message(
                    ...
                    text=f"<b>Вам отменили запись</b>\n"
                    f"Дата: <b>{date_}</b>\n"
                    f"Время: <b>{formatted_time}</b> (МСК)\n"
...
async def cmd_start(message: Message):
    ...
    user: TelegramOut = await BotService.find_user_by_tg_id(
...
async def my_clients(message: Message):
    ...
    res: list[BookingOut] = await BotService.get_all_bookings(
        user_id=user.id, date=datetime.now(tz=moscow_tz).date()
    )
...
async def get_clients_by_date(message: Message, command: CommandObject):
    ...
    if not user:
        ...
        try:
            ...
            await message.answer(
                f"Неверная дата, введите в формате: День.Месяц.Год\nПример: /date 01.01.2024"
            )
        else:
            clients: BookingOut = await BotService.get_booking(
                user_id=user.id, date=date_object
            )
...
async def connect_telegram(message: Message):
    ...
    await message.answer(
        "Нажмите на кнопку чтобы привязать ваш аккаунт", reply_markup=inline_kb
    )
